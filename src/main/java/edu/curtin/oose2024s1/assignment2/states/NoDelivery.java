...
public class NoDelivery implements ShopState //State for when shop cant accept delivery
{ 
    private static final Logger LOGGER = Logger.getLogger(NoDelivery.class.getName());
    @Override 
    ...
    {
        if(shop.getTotalBikes() <= 90 && shop.getCash() >= 10000) //again check for space and cash
        {
            shop.setState(new AllAvailable()); //Change the state to AllAvailable if there is enough space and cash 
            LOGGER.info("Enough space and cash for Delivery: State Changed to AllAvailable");
            shop.delivery();
        }
        else if(shop.getTotalBikes() > 90) //If there is not enough space for delivery
        {
            ...
            LOGGER.info("Not enough space for Delivery, Delivery not accepted");
        }
        else if(shop.getCash() < 10000) //If there is not enough cash for delivery
        {
            shop.notifyEventObservers("!!!!!!!!!!!!!!!!\nFAILURE: Not enough cash"); //Notify the observers of the failure
            shop.failOccured(); //Increment the fail count
            LOGGER.info("Not enough cash for Delivery, Delivery not accepted");
        }
    }
    ...
    public void dropOff(BikeShop shop, Bike bike) //Method to drop off a bike
    {
        ...
        {
            shop.setTotalBikes(shop.getTotalBikes() + 1); //Increment the total number of bikes
            shop.setNumServBikes(shop.getNumServBikes() + 1); //Increment the number of bikes being serviced
            shop.addToBikeList(bike); //Add the bike to the bike list
